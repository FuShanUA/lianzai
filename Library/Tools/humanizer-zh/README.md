...
2. 将 `Humanizer-zh` 文件夹复制到 Claude Code 的 skills 目录：
   ...
   - **Windows**: `/Users/shanfu\.claude\skills\`
...
- 此外、至关重要、深入探讨、强调、深挖 (AI 滥用黑话，原意是坚持、探究、深入分析等，尽量不用)
- 持久的、增强、培养、获得
- 突出、相互作用、复杂/复杂性
- 格局（抽象名词）、关键性的、展示
- 织锦（抽象名词）、证明、强调
...
### 核心术语翻译纠偏
- **Context (优先翻译为“上下文”)**：在技术讨论、计算机、代码、LLM等相关语境中，`context` (或 `Context`) 绝大多数情况下应翻译为“上下文”而不是“语境”。只有在明确指代社会、文化或语言背景等宏观语境时才考虑翻译为“语境”。
